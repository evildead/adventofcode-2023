...
import { DamagedSprings, ArrangementsResponseType } from './damagedSprings';
...
As you turn to enter, the researcher stops you. "Wait - I thought you were looking for the
hot springs, weren't you?" You indicate that this definitely looks like hot springs to you.
...
not until we get more lava to heat our forges. And our springs. The springs aren't very springy
unless they're hot!"
...
broken springs (in that order) can appear in that row: the first three unknown springs must be broken,
...
and each ?? must hide exactly one of the two broken springs. (Neither ?? could be both broken springs or
they would form a single contiguous group of two; if that were true, the numbers afterward would have
been 2,3 instead.) Since each ?? can either be #. or .#, there are four possible arrangements of springs.
...
However, the remaining run of unknown spring conditions have many different ways they could hold groups
...
records list. When you examine the records, you discover that they were actually folded up this whole time!
...
export function countArrangementsPart1(filePath: string): ArrangementsResponseType {
  const fileLines = getTextFileAsListOfLines(filePath);
  const damagedSprings = new DamagedSprings(fileLines);
  const result = damagedSprings.computeArrangementsPart1();
  return result;
}

export function startDay12() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 12');

  // PART 1
  // const resPart1 = countArrangementsPart1('data/day12/testInput01.txt');
  const resPart1 = countArrangementsPart1('data/day12/input01.txt');
  consoleLogger.debug(`PART 1: ${JSON.stringify(resPart1, null, 2)}`);
  consoleLogger.info(`PART 1: ${resPart1.sumOfArrangements}`);
