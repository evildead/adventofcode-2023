...
import { LeftRightSteps } from './leftRightSteps';
...
export function leftRightStepsPart1(filePath: string): number {
  const fileLines = getTextFileAsListOfLines(filePath);
  const leftRightSteps = new LeftRightSteps(fileLines);
  const result = leftRightSteps.computeNumberOfStepsPart1();
  return result;
}

export function leftRightStepsPart2(filePath: string): number {
  const fileLines = getTextFileAsListOfLines(filePath);
  const leftRightSteps = new LeftRightSteps(fileLines);
  const result = leftRightSteps.computeNumberOfStepsPart2CorrectButTooSlow();
  return result;
}

export function startDay08() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 8');

  // PART 1
  // const resPart1 = leftRightStepsPart1('data/day08/testInput01.txt');
  // const resPart1 = leftRightStepsPart1('data/day08/testInput02.txt');
  const resPart1 = leftRightStepsPart1('data/day08/input01.txt');
  consoleLogger.info(`PART 1: ${resPart1}`);

  // PART 2
  // const resPart2 = leftRightStepsPart2('data/day08/testInput03.txt');
  // const resPart2 = leftRightStepsPart2('data/day08/input01.txt');
  // consoleLogger.info(`PART 2: ${resPart2}`);
