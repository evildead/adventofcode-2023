...
import { ParabolicReflectorDish, TotalAmountOfLoad } from './parabolicReflectorDish';
...
--- Part Two ---
The parabolic reflector dish deforms, but not in a way that focuses the beam. To do that, you'll need
to move the rocks to the edges of the platform. Fortunately, a button on the side of the control panel
...
then east. After each tilt, the rounded rocks roll as far as they can before the platform tilts in the
next direction. After one cycle, the platform will have finished rolling the rounded rocks in those
...
export function totalLoadOfNorthSupportBeamsPart1(filePath: string) {
  const fileLines = getTextFileAsListOfLines(filePath);
  const parabolicReflectorDish = new ParabolicReflectorDish(fileLines);
  const result = parabolicReflectorDish.computeLoadOfNorthSupportBeamsPart1();
  return result;
}

export function startDay14() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 14');

  // PART 1
  // const resPart1 = totalLoadOfNorthSupportBeamsPart1('data/day14/testInput01.txt');
  const resPart1 = totalLoadOfNorthSupportBeamsPart1('data/day14/input01.txt');
  consoleLogger.debug(`PART 1: ${JSON.stringify(resPart1, null, 2)}`);
  consoleLogger.info(`PART 1: ${resPart1.totalAmount}`);
