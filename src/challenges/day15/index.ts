...
You find a door under a large sign that says "Lava Production Facility" and next to a smaller sign that
says "Danger - Personal Protective Equipment required beyond this point".

As you step inside, you are immediately greeted by a somewhat panicked reindeer wearing goggles and a
loose-fitting hard hat. The reindeer leads you to a shelf of goggles and hard hats (you quickly find
some that fit) and then further into the facility. At one point, you pass a button with a faint snout
mark and the label "PUSH FOR HELP". No wonder you were loaded into that trebuchet so quickly!

You pass through a final set of doors surrounded with even more warning signs and into what must be the
room that collects all of the light from outside. As you admire the large assortment of lenses available
to further focus the light, the reindeer brings you a book titled "Initialization Manual".
...
After following these steps for each character in the string in order, the current value is the output
...
passing through the box. The side of each box has a panel that opens to allow you to insert or remove
lenses as necessary.
...
Within each box, lenses are listed from front to back; each lens is shown as its label and focal length
in square brackets.
...
export function sumHashAlgorithmResultsPart1(filePath: string): HashAlgorithmResults {
  const fileLines = getTextFileAsListOfLines(filePath);
  const lensLibrary = new LensLibrary(fileLines[0]);
  const result = lensLibrary.sumHashAlgorithmResultsPart1();
  return result;
}
...
export function startDay15() {
  ...
  asciiArtLogger.info('Day 15');

  // PART 1
  // const resPart1 = sumHashAlgorithmResultsPart1('data/day15/testInput01.txt');
  const resPart1 = sumHashAlgorithmResultsPart1('data/day15/input01.txt');
