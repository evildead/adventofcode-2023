...
import { LinesOfReflection } from './linesOfReflection';
...
the valley between them is completely full of large mirrors. Most of the mirrors seem to be aligned
...
the pattern and must match exactly:
...
to be valid. (The old reflection line won't necessarily continue being valid after the smudge is fixed.)
...
and 4 now exists. Row 7 has no corresponding reflected row and can be ignored, but every other row matches
exactly: row 1 matches row 6, row 2 matches row 5, and row 3 matches row 4.
...
export function findLinesOfReflectionPart1(filePath: string) {
  const fileLines = getTextFileAsListOfLines(filePath);
  const linesOfReflection = new LinesOfReflection(fileLines);
  const result = linesOfReflection.computeLinesOfReflectionScorePart1();
  return result;
}

export function startDay13() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 13');

  // PART 1
  // const resPart1 = findLinesOfReflectionPart1('data/day13/testInput01.txt');
  const resPart1 = findLinesOfReflectionPart1('data/day13/input01.txt');
  consoleLogger.debug(`PART 1: ${JSON.stringify(resPart1, null, 2)}`);
  consoleLogger.info(`PART 1: ${resPart1.totalScore}`);
