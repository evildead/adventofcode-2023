...
import { Galaxies, GalaxiesDistancesResponseType } from './galaxies';
...
export function sumOfGalaxiesDistancesPart1(filePath: string): GalaxiesDistancesResponseType {
  const fileLines = getTextFileAsListOfLines(filePath);
  const galaxies = new Galaxies(fileLines);
  const result = galaxies.computeGalaxiesDistancesPart1();
  return result;
}

export function startDay11() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 11');

  // PART 1
  // const resPart1 = sumOfGalaxiesDistancesPart1('data/day11/testInput01.txt');
  const resPart1 = sumOfGalaxiesDistancesPart1('data/day11/input01.txt');
  consoleLogger.debug(`PART 1: ${JSON.stringify(resPart1)}`);
  consoleLogger.info(`PART 1: ${resPart1.sumOfDistances}`);
