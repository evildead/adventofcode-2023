...
import { TheContraption } from './theContraption';

/*
--- Day 16: The Floor Will Be Lava ---
With the beam of light completely focused somewhere, the reindeer leads you deeper still into the
Lava Production Facility. At some point, you realize that the steel facility walls have been replaced
with cave, and the doorways are just cave, and the floor is cave, and you're pretty sure this is
...
cavern wall closest to the facility and pouring all of its energy into a contraption on the opposite side.
...
.|....-|.\
..//.|....
The beam enters in the top-left corner from the left and heading to the right. Then, its behavior depends
...
mirror's column, while a rightward-moving beam that encounters a \ mirror would continue downward from
the mirror's column.
If the beam encounters the pointy end of a splitter (| or -), the beam passes through the splitter as if
the splitter were empty space. For instance, a rightward-moving beam that encounters a - splitter would
...
analyzing the current situation. With the beam starting in the top-left heading right, how many tiles end
up being energized?
...
export async function countEnergizedTiles(filePath: string): Promise<number> {
  const fileLines = getTextFileAsListOfLines(filePath);
  const theContraption = new TheContraption(fileLines);
  const result = await theContraption.computeEnergizedTiles();
  return result;
}

export async function startDay16() {
  const asciiArtLogger = getAsciiArtLogger('debug', 'Doom');
  const consoleLogger = getConsoleLogger('debug');
  asciiArtLogger.info('Day 16');

  // PART 1
  // const resPart1 = await countEnergizedTiles('data/day16/testInput01.txt');
  const resPart1 = await countEnergizedTiles('data/day16/input01.txt');
  // consoleLogger.debug(`PART 1: ${JSON.stringify(resPart1, null, 2)}`);
  consoleLogger.info(`PART 1: ${resPart1}`);
